...
const StepThree = ({
  ...
  const [specification_nameSerial, setSpecification_nameSerial] = useState("");
  const [saveData, setAllSaveData] = useState(
    Array.isArray(jananiStepThreeData) ? jananiStepThreeData : []
  );
  ...
  const handleDataPost = async (data) => {
    ...
    // check specification_name is exist
    if (
      saveData.some((entry) => entry.specification_name === specification_name)
    ) {
      toast.error("Specification already exists");
      return;
    }

    if (Array.isArray(data.specificationValue)) {
      const sendData = {
        specification_id,
        specification_name,
        specification_details: data.specificationValue.map((item, index) => ({
          specification_details_id: selectedOptions[index]?.filter_id,
          specification_details_name: item?.specification_details_name,
          specification_details_value: item?.specification_details_value,
        })),
      };
      setAllSaveData([...saveData, sendData]);
    }
    reset();
  ...
  const handleDeleteOldSpecification = (specification_id) => {
    setAllSaveData(
      saveData?.filter((item) => item.specification_id !== specification_id)
    );
    setNextButtonStatus(true);
  };
    ...
    const updatedSaveData = saveData?.map((item) => {
      ...
      if (item?.specification_id === specification_id) {
        ...
        item.specification_details = item.specification_details.filter(
          (detail) =>
            detail?.specification_details_id !== specification_details_id
        );
      }
      return item;
    });
  ...
  // submit data
  const handleNext = () => {
    const alldataString = JSON.stringify(saveData);
    localStorage.setItem("jananiStepThreeData", alldataString);
  ...
  return (
    <div>
          ...
          {saveData?.map((data) => (
            <div key={data.specification_id}>
              <div className="flex items-center justify-between text-gray-800 bg-gray-300 py-2 px-4">
                ...
                <MdDeleteForever
                  onClick={() =>
                    handleDeleteOldSpecification(data?.specification_id)
                  }
                  ...
                  {data?.specification_details?.map((specidata) => (
                    <tr key={specidata.specification_details_name}>
                      <td className="py-2 px-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                        {specidata?.specification_details_name}
                      </td>
                        ...
                        <MdDeleteForever
                          onClick={() =>
                            handleDeleteOldSpecification_details_name(
                              ...
                              specidata?.specification_details_id
                            )
                          }
                          className="cursor-pointer text-red-500 hover:text-red-300"
                          size={25}
      ...
      <form onSubmit={handleSubmit(handleDataPost)}>
        <div className="mt-10 border p-5 border-gray-300 rounded-md mb-10">
            ...
            <Select
              ...
              options={specifications?.data
                ?.filter(
                  (option) =>
                    !saveData?.some(
                      (item) => item?.specification_id === option?._id
                    )
                )
                ?.sort((a, b) =>
                  a.specification_serial > b.specification_serial ? 1 : -1
                )}
              required
              getOptionLabel={(x) =>
                x?.specification_serial
                  ? x?.specification_serial + ". " + x?.specification_name
                  : specification_nameSerial + ". " + x?.specification_name
              }
              getOptionValue={(x) => x?._id}
              onChange={(selectedOption) => {
                setSpecification_id(selectedOption?._id);
                setSpecification_name(selectedOption?.specification_name);
                setSpecification_nameSerial(
                  selectedOption?.specification_serial
                );
                          ...
                          (option) =>
                            !saveData.some((item) =>
                              item.specification_details.some(
                                (filterFind) =>
                                  filterFind.specification_details_name ===
                                  option.filter_name
                              )
                            )
                        )
