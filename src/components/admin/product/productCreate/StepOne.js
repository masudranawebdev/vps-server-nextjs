...
import { useCheckIdAndSkuMutation } from "@/redux/feature/product/productApi";
...
const StepOne = ({
  ...
  const [category_id, setCategory_id] = useState(
    stepOneDefaultdata?.category_id ? stepOneDefaultdata?.category_id : ""
  );

  const [category_name, setCategory_name] = useState(
    stepOneDefaultdata?.category_name ? stepOneDefaultdata?.category_name : ""
  );
  ...
  const [brand_id, setBrand_id] = useState(
    stepOneDefaultdata?.brand_id ? stepOneDefaultdata?.brand_id : ""
  );
  const [brand_name, setBrand_name] = useState(
    stepOneDefaultdata?.brand_name ? stepOneDefaultdata?.brand_name : ""
  );
  ...
  const child_categoryNameValue = stepOneDefaultdata?.child_category_name
    ? stepOneDefaultdata?.child_category_name
    : "";
  const child_categoryNameId = stepOneDefaultdata?.child_category_id
    ? stepOneDefaultdata?.child_category_id
    : "";

  const brandNameValue = stepOneDefaultdata?.brand_name
    ? stepOneDefaultdata?.brand_name
    : "";
  const brandNameId = stepOneDefaultdata?.brand_id
    ? stepOneDefaultdata?.brand_id
    : "";
  ...
  const [checkProductIdAndSkuUnique] = useCheckIdAndSkuMutation(); //check product id and sku
  ...
  }, [brands?.data, category_id, sub_category_id, child_category_id]);
  ...
  const handleDataPost = (data) => {
    ...
    checkProductIdAndSkuUnique(sendIdSku).then((result) => {
      if (result?.data?.statusCode == 200 && result?.data?.success == true) {
        ...
        localStorage.setItem("jananiStepOneData", alldataString);
        if (currentStep === steps.length) {
          setComplete(true);
        } else {
          setCurrentStep((prev) => prev + 1);
