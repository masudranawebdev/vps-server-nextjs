...
const StepTwo = ({
  ...
  const [oldFilterSub_Filter, setOldFilterSub_Filter] = useState(
    jananiStepTwoData ? jananiStepTwoData : []
  );
              ...
              <table className="min-w-full divide-y-2 divide-gray-200 text-sm border border-gray-300 mt-4 rounded-xl">
                ...
                <tbody className="divide-y divide-gray-200">
                  {oldFilterSub_Filter?.map((filterSubFilter) => (
                    <tr key={filterSubFilter?.filter_name}>
          ...
          <div>
                ...
                ?.filter(
                  (option) =>
                    !oldFilterSub_Filter?.some(
                      (item) => item?.filter_id === option?._id
                    )
                )
            ...
            {/* <Select
              onChange={(selectedOption) => {
                ...
                setFilter_nameSerial(selectedOption?.filter_serial);
                setIsChild_FilterOpen(false);
                setChild_Filter_id();
                setChild_filter_name();
                setFilterStatus(true);
