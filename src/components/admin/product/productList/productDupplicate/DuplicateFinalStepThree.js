...
const DuplicateStepThree = ({
  ...
  //     if (
  //         saveData.some((entry) => {
  //             return entry?.specification_id?.specification_name === specification_name;
  //         })
  //     ) {
  ...
  const handleDataPost = async (data) => {
    ...
    if (existingSpecificationIndex !== -1) {
      // If the specification exists, append the new specification details
      const updatedSaveData = [...saveData];
      const existingSpecification = updatedSaveData[existingSpecificationIndex];
      const newSpecificationDetails = data.specificationValue.map(
        (item, index) => ({
          specification_details_id: {
            _id: selectedOptions[index]?.filter_id,
            filter_name: item?.specification_details_name,
...
export default DuplicateStepThree;
