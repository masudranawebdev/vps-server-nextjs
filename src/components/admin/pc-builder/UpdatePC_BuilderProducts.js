...
const UpdatePC_BuilderProducts = ({
  category_id,
  addProduct,
  setAddProduct,
  setDeleteProduct,
  ...
  const { data: products = [], isLoading } = useQuery({
    queryKey: [
      `/api/v1/pc_builder/dashboard/get_product_for_add?category_id=${category_id}&page=${page}&limit=${rows}&searchTerm=${searchTerm}`,
    ],
    queryFn: async () => {
      const res = await fetch(
        `${BASE_URL}/pc_builder/dashboard/get_product_for_add?category_id=${category_id}&page=${page}&limit=${rows}&searchTerm=${searchTerm}`,
        {
          headers: {
  ...
  const handleRemoveCampaignProdduct = (product) => {
    ...
    const isProductExist = deleteProduct?.some((item) => item === product);

    // If the product doesn't exist, add it to deleteProduct array
    if (!isProductExist) {
      setDeleteProduct((prev) => [...prev, product]);
    }
  };
  return (
    <div>
              ...
              {allProducts?.map((product) => {
                ...
                return (
                  <tr key={product?._id}>
                    <td className="whitespace-nowrap px-4 py-2 flex justify-center mt-3 gap-4">
                      {isProductAdded ? (
                        // If the product is already added, show a different icon or style
                        <MdAddBox
                        ...
                        // If the product is not added yet, show the add button normally
                        <MdAddBox
                          onClick={() => {
...
export default UpdatePC_BuilderProducts;
