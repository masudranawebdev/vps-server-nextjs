...
const DeleteBanner = ({
  refetch,
  setOpenDeleteBannerModal,
  openDeleteBannerModalId,
  openDeleteBannerModalPublic_Id,
}) => {
  const [deleteBannerType] = useDeleteBannerMutation(); //delete banner
  const [loading, setLoading] = useState(false);

  const handleDeleteCategory = () => {
    setLoading(true);
    const sendData = {
      _id: openDeleteBannerModalId,
      public_id: openDeleteBannerModalPublic_Id,
    };
    deleteBannerType(sendData).then((result) => {
      ...
      <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-full lg:w-10/12 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button className="btn bg-white border p-1 hover:bg-primaryColor hover:text-white">
            <RxCross1
              onClick={() => setOpenDeleteBannerModal(false)}
              size={25}
            ></RxCross1>
          </button>
        </div>
