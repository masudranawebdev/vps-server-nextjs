...
const EditCouponProduct = ({ addProduct, setAddProduct }) => {
  ...
  const handleAddCampaignProdduct = (product) => {
    setAddProduct((prev) => [...prev, { coupon_product_id: product }]);
  };
  ...
  allProducts?.data?.forEach((product) => {
    const isProductAdded = addProduct?.some(
      (addedProduct) => addedProduct?.coupon_product_id?._id === product?._id
    );
              ...
              <tbody className="divide-y divide-gray-200">
                {addProduct?.map((product) => (
                  <tr key={product?._id}>
                    ...
                    <td className="whitespace-nowrap px-4 py-2">
                      {product?.coupon_product_id?.product_brand_id?.brand_name}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <table className="min-w-full divide-y-2 divide-gray-200 text-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {remaingProducts?.map((product) => {
                return (
                  <tr key={product?.coupon_product_id?._id}>
                    <td className="whitespace-nowrap px-4 py-2 flex justify-center mt-3 gap-4">
                      <MdAddBox
                        onClick={() => {
...
export default EditCouponProduct;
