...
const EditCampaignProduct = ({ addProduct, setAddProduct }) => {
  ...
  const handleAddCampaignProdduct = (product) => {
    setAddProduct((prev) => [...prev, { campaign_product_id: product }]);
  };
  // delete selected product
  const handleRemoveCampaignProdduct = (product) => {
    const newProduct = addProduct?.filter(
      (oldProduct) =>
        oldProduct?.campaign_product_id?._id !==
        product?.campaign_product_id?._id
    );
            ...
            <tbody className="divide-y divide-gray-200">
              {allProducts?.data?.map((product) => {
                const isProductAdded = addProduct.some(
                  (addedProduct) =>
                    addedProduct?.campaign_product_id?._id === product?._id
                );
                return (
                  <tr key={product?._id}>
                    <td className="whitespace-nowrap px-4 py-2 flex justify-center mt-3 gap-4">
...
export default EditCampaignProduct;
