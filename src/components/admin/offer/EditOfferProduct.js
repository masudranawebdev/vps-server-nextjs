...
const EditOfferProduct = ({ addProduct, setAddProduct }) => {
  ...
  const handleAddOfferProdduct = (product) => {
    setAddProduct((prev) => [...prev, { offer_product_id: product }]);
  };
  // delete selected product
  const handleRemoveOfferProdduct = (product) => {
    const newProduct = addProduct?.filter(
      (oldProduct) =>
        oldProduct?.offer_product_id?._id !== product?.offer_product_id?._id
    );
                ...
                {addProduct?.map((product) => (
                  <tr key={product?._id}>
                    <td className="whitespace-nowrap px-4 py-2 flex justify-center mt-3 gap-4">
                      <CiSquareRemove
                        onClick={() => {
                          handleRemoveOfferProdduct(product);
                        }}
                    ...
                    <td className="whitespace-nowrap px-4 py-2">
                      {product?.offer_product_id?.product_brand_id?.brand_name}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <tbody className="divide-y divide-gray-200">
              {allProducts?.data?.map((product) => {
                const isProductAdded = addProduct.some(
                  (addedProduct) =>
                    addedProduct?.offer_product_id?._id === product?._id
                );
                return (
                  <tr key={product?._id}>
                    <td className="whitespace-nowrap px-4 py-2 flex justify-center mt-3 gap-4">
                      {isProductAdded ? (
                        ...
                        <MdAddBox
                          onClick={() => {
                            handleAddOfferProdduct(product);
                          }}
