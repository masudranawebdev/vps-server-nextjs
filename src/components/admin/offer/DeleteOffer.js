...
import { useDeleteOfferMutation } from "@/redux/feature/offer/offerApi";
import { useState } from "react";
import { RxCross1 } from "react-icons/rx";
import { toast } from "react-toastify";

const DeleteOffer = ({
  setOpenDeleteOfferModal,
  openDeleteOfferModalValue,
  refetch,
}) => {
  const [deleteOffer] = useDeleteOfferMutation(); //delete offer
  const [loading, setLoading] = useState(false);

  const handleDeleteOffer = () => {
    setLoading(true);
    const sendData = {
      _id: openDeleteOfferModalValue?._id,
      public_id: openDeleteOfferModalValue?.public_id,
    };
    deleteOffer(sendData).then((result) => {
      ...
      <div className="relative overflow-hidden text-left bg-white rounded-lg shadow-xl w-full lg:w-10/12 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button className="btn bg-white border p-1 hover:bg-primaryColor hover:text-white">
            <RxCross1
              onClick={() => setOpenDeleteOfferModal(false)}
              size={25}
            ></RxCross1>
          </button>
        </div>
        ...
        <h2 className="text-red-500 mt-8 text-xl">
          Are you sure you want to delete this Offer name:{" "}
          {openDeleteOfferModalValue?.offer_title} ?
        </h2>
